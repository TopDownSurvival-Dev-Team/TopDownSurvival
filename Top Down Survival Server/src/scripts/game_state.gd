...
onready var container_menu = $HUD/ContainerMenu
...
func save_world():
	...
	for block_pos in block_data:
		var data = {
			"entity_type": "block",
			"position": block_pos,
			"entity_info": {"block_id": block_data[block_pos]}
		}
		world_data.append(data)
	...
	for item in items.get_children():
		var data = {
			...
			"entity_info": {"quantity": item.quantity}
		}
		world_data.append(data)
...
func on_item_dropped(item_id: String, quantity: int, player_id: int):
	...
	var player_direction = Vector2(cos(player.rotation), sin(player.rotation))
